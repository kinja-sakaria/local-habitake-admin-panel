  ...
  return (
    <Box>
      <Typography variant="h5" fontWeight={700} color="secondary.800" px={2.5} pt={2.5}>
        {activeTab === 0 && (
          <>
            {user.role === 'Seller' && 'Seller - Listed Properties'}
            {user.role === 'Agent' && 'Agent - Listed Properties'}
          </>
        )}

        {activeTab === 1 && 'Agent - Listed Properties'}

        {activeTab === 2 && 'Visited Properties List'}
      </Typography>
      {/* Top bar: Search and Sort */}
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2} p={2.5} bgcolor="white">
        <TextField
